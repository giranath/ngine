...
private:
    [[nodiscard]] pugi::xml_node find_property_node(const ng::name& name) const;
...
public:
    ...
    [[nodiscard]] ng::name name() const override final;
    [[nodiscard]] uint8_t version() const override final;

    [[nodiscard]] std::optional<uint64_t> deserialize_uint64(const ng::name& name, uint64_t def) const override final;
    [[nodiscard]] std::optional<int64_t> deserialize_int64(const ng::name& name, int64_t def) const override final;
    [[nodiscard]] std::optional<bool> deserialize_bool(const ng::name& name, bool def) const override final;
    [[nodiscard]] std::optional<double> deserialize_double(const ng::name& name, double def) const override final;
    [[nodiscard]] std::optional<std::string> deserialize_string(const ng::name& name, std::string_view def) const override final;

    void begin_object(const ng::name& name) override final;
    void end_object() override final;

    void begin_array(const ng::name& name) override final;
    void end_array() override final;

    void begin_dictionary(const ng::name& name) override final;
